...
//统计器
var Stater = function(title, dict, openId, activityId, channelId){
    this.stTitle = title;
    this.stDict = dict;
    this.openId = openId;
...
Stater.prototype.stat = function(code){
    try{
        MtaH5.clickStat(code+'');
    }catch(e){
        
    }
    
    try{
        var n = parseInt(code / 1000);
        switch(n){
        case 1:
            this.countPage(this.stDict[code], code);
            break;
        case 2:
            this.countButton(this.stDict[code], code);
            break;
        case 9:
            this.countWXShare();
            break;
...
// 统计页面
Stater.prototype.countPage = function(pageName, pageNo){
    var title = this.stTitle + "(页面-"+pageName+")"
    var type = "03"
    var subType = "0301"
    ...
    var param = {
        "openid": this.openId,
        "name" : title,
        "code" : "ACTIVITY"+this.activityId+"_"+subType+"_INDEX"+pageNo,
        "type" : type,
        "subtype" : subType,
    ...
    this.MiguStatistics(param);
};

// 统计按钮
Stater.prototype.countButton = function(buttonName, buttonNo){
    var title = this.stTitle + "(按钮-"+buttonName+")"
    var type = "03"
    var subType = "0302"
    ...
    var param = {
        "openid": this.openId,
        "name" : title,
        "code" : "ACTIVITY"+this.activityId+"_"+subType+"_INDEX"+buttonNo,
        "type" : type,
        "subtype" : subType,
        "channelId" : this.channelId,
        "docid" : "13" //活动页面
...
// 统计分享
Stater.prototype.countWXShare = function(){
    var title = this.stTitle + "(微信分享)"
    var type = "03"
    var subType = "0303"
    ...
    var param = {
        "openid": this.openId,
        "name" : title,
        "code" : "ACTIVITY"+this.activityId+"_"+subType+"_INDEX",
        "type" : type,
        "subtype" : subType,
        "channelId" : this.channelId,
        "docid" : "13" //活动页面
...
Stater.prototype.MiguStatistics = function(param){
    $.ajax({
        url: '/wechat/service/statistics.do',
        type: "POST",
        dataType: "json",
        global: false,
